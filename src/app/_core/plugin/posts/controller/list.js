...
const purge = (req, res) => {
    ...
    Parse.Cloud.run('nonce_get', {id: nonce}).then(() => {
        ...
        return Parse.Cloud.run('content_purge', {type: 'post'}, {sessionToken: req.jam.sessionToken});
    }).then((result) => {
        output.data = result;
...
exports.use = (req, res, next) => {
    ...
    Parse.Cloud.run('nonce_create').then((result) => {
        // Get widgets
        core.add_widgets('post-list', req);
...
exports.all = (req, res) => {
    ...
    Parse.Cloud.run('content_get_posts', opt, {sessionToken: req.jam.sessionToken}).then((results) => {
        req.jam.posts['pagination'] = results.pagination;
        ...
        req.jam.posts['query']    = opt;
        req.jam.posts['list']     = output;
        req.jam.posts['filtered'] = filtered;
